...
export const WorkTypeManager: React.FC<WorkTypeManagerProps> = ({ workTypes, users = [], onAdd, onUpdate, onDelete, currentUser }) => {
  ...
  const [newQuoteDeleteAllowedRoles, setNewQuoteDeleteAllowedRoles] = useState<UserRole[]>([]); // Rôles pour supprimer le devis
  ...
  const [editQuoteDeleteAllowedRoles, setEditQuoteDeleteAllowedRoles] = useState<UserRole[]>([]);
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const newType: WorkType = {
            ...
            quoteDeleteAllowedRoles: newQuoteDeleteAllowedRoles
          };
  ...
  const startEdit = (type: WorkType) => {
    ...
    const quoteDeleteRoles = type.quoteDeleteAllowedRoles && Array.isArray(type.quoteDeleteAllowedRoles) ? type.quoteDeleteAllowedRoles : [];
    setEditQuoteDeleteAllowedRoles(quoteDeleteRoles);
  };
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const updatedType: WorkType = {
            ...
            quoteDeleteAllowedRoles: editQuoteDeleteAllowedRoles
          };
        ...
        <div className="md:col-span-1 space-y-4">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            <form onSubmit={handleAdd} className="space-y-3">
              ...
              {/* Section pour les rôles de validation de demande */}
              <div className="mt-3 pt-3 border-t border-green-100">
                <p className="text-[10px] font-black text-green-600 uppercase tracking-widest mb-2">✓ Rôles - Validation Demande :</p>
                <div className="relative">
                  <button
                    type="button"
              ...
              {/* NOUVEAU: Section pour les rôles de suppression demande */}
              <div className="mt-3 pt-3 border-t border-rose-100">
                <p className="text-[10px] font-black text-rose-600 uppercase tracking-widest mb-2">🗑️ Rôles - Suppression Demande :</p>
                <div className="relative">
                  <button
                    type="button"
                  ...
                  <li key={type.id} className="px-6 py-4 flex flex-col hover:bg-blue-50/30 transition-colors">
                        ...
                        {editingId === type.id ? (
                          ...
                          <>
                            <div className="flex-grow">
                              ...
                              <div className="mt-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                                {/* Création Demande */}
                                ...
                                <div className="bg-rose-50/50 p-3 rounded-lg border border-rose-100">
                                  <div className="text-[10px] font-black text-rose-600 uppercase tracking-widest mb-2 flex items-center gap-1">
                                    <span>🗑️ Suppression Demande</span>
                                  </div>
                                ...
                                <div className="bg-rose-50/50 p-3 rounded-lg border border-rose-100">
                                  <div className="text-[10px] font-black text-rose-600 uppercase tracking-widest mb-2 flex items-center gap-1">
                                    <span>🗑️ Suppression Devis</span>
                                  </div>
                                  {type.quoteDeleteAllowedRoles && type.quoteDeleteAllowedRoles.length > 0 ? (
                                    <div className="space-y-3">
                                      {type.quoteDeleteAllowedRoles.map((role, i) => {
                                        const roleUsers = users.filter(u => u.role === role);
                                        return (
                                          <div key={i} className="text-xs">
                                            <div className="font-bold text-rose-800/80 mb-1.5 border-b border-rose-100/50 pb-1">{role}</div>
                    ...
                    {editingId === type.id && userCanEdit && (
                      <div className="mt-3 pt-3 border-t border-gray-100 space-y-3">
                        ...
                        {/* NOUVEAU: Rôles suppression demande */}
                        ...
                        {/* NOUVEAU: Rôles suppression devis */}
                        <div className="pt-3 border-t border-rose-100">
                          <p className="text-[10px] font-black text-rose-600 uppercase tracking-widest mb-2">🗑️ Rôles - Suppression Devis :</p>
                          <div className="relative">
                            <button
                              type="button"
                              className="w-full p-2 border border-rose-300 rounded-md text-xs bg-white text-left flex justify-between items-center"
                            ...
                            >
                              {editQuoteDeleteAllowedRoles.length > 0 ? `${editQuoteDeleteAllowedRoles.length} rôle(s) sélectionné(s)` : 'Sélectionner des rôles...'}
                              <svg className="w-4 h-4 text-gray-400 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path>
                              </svg>
                            </button>
                              ...
                              {Object.values(UserRole).map((role, index) => (
                                <label key={index} className="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                  <input
                                    type="checkbox"
                                    className="rounded text-rose-600 focus:ring-rose-500"
                                    checked={editQuoteDeleteAllowedRoles.some(allowedRole => allowedRole === role)}
                                    onChange={(e) => {
                                      if (e.target.checked) {
                                        if (!editQuoteDeleteAllowedRoles.some(allowedRole => allowedRole === role)) {
                                          setEditQuoteDeleteAllowedRoles([...editQuoteDeleteAllowedRoles, role]);
                                        }
                                      } else {
                                        setEditQuoteDeleteAllowedRoles(editQuoteDeleteAllowedRoles.filter(allowedRole => allowedRole !== role));
                                      }
                                    }}
                                  />
                                  <span className="ml-2 text-xs">{role}</span>
                          ...
                          <div className="mt-2 flex flex-wrap gap-1">
                            {editQuoteDeleteAllowedRoles.map((role, index) => (
                              <span key={index} className="inline-flex items-center gap-1 bg-rose-100 text-rose-800 px-2 py-1 rounded-full text-[8px] font-bold">
                                {role}
                                <button
                                  type="button"
                                  className="text-rose-600 hover:text-rose-800"
                                  onClick={() => {
                                    setEditQuoteDeleteAllowedRoles(editQuoteDeleteAllowedRoles.filter(r => r !== role));
                                  }}
                                >
                                  ×
                                </button>
