...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 relative overflow-hidden">
      ...
      <div className="absolute -bottom-8 -right-4 w-72 h-72 bg-indigo-200 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-2000"></div>

      <div className="max-w-md w-full p-8 bg-white shadow-2xl border border-gray-100 rounded-3xl relative z-10 animate-in fade-in zoom-in duration-500">
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tighter">AEP Manager</h2>
          <p className="text-gray-500 text-sm font-medium mt-1">Connexion à la base GestionEau</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          {error && (
            <div className={`${blockedUntil ? 'bg-orange-100 border-orange-300 text-orange-700' : 'bg-red-100 border-red-300 text-red-700'} border text-xs p-3 rounded-lg text-center font-bold uppercase tracking-widest animate-shake`}>
              {error}
            </div>
          )}
          
          {blockedUntil && remainingTime > 0 && (
            <div className="bg-orange-100 border border-orange-300 text-orange-700 text-xs p-3 rounded-lg text-center">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-[10px] font-black text-gray-600 uppercase tracking-widest mb-1.5 ml-1">Utilisateur</label>
              <input 
                type="text" 
                required
                disabled={!!blockedUntil}
                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all sm:text-sm disabled:opacity-50"
            ...
            <div>
              <label className="block text-[10px] font-black text-gray-600 uppercase tracking-widest mb-1.5 ml-1">Mot de passe</label>
              <input 
                type="password" 
                required
                disabled={!!blockedUntil}
        ...
        <div className="mt-10 text-center">
          <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.3em]">Atlas Secure Access v2.4</p>
        </div>
      </div>
    </div>
  );
